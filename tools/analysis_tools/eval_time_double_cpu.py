...
def measure_inference_speed(cfg, max_iter, log_interval,
                            is_fuse_conv_bn):
    # set cudnn_benchmark
    ...
    params = sum(p.numel() for p in model.parameters() if p.requires_grad) / 10**6
    print(params)
    ...
    for i, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            model(return_loss=False, rescale=True, img=data['img'], img_thermal=data['img_thermal'], img_metas=data['img_metas'][0].data)
...
def repeat_measure_inference_speed(cfg,
    ...
    for _ in range(repeat_num):
        ...
        fps_list.append(
            measure_inference_speed(cp_cfg, max_iter, log_interval,
                                    is_fuse_conv_bn))
...
def main():
    ...
    repeat_measure_inference_speed(cfg, args.max_iter,
