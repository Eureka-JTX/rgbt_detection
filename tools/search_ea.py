...
class EvolutionSearcher(object):
    ...
    def __init__(self, args):
        ...
        seed(0)
        self.flop_upper_bound = 289.61 
        self.param_upper_bound = 138.46
        self.log_path = args.checkpoint[:-4]
        self.logfile = args.checkpoint[:-4] + '_ea_fp{}_{}.log'.format(self.flop_upper_bound, times)
        # self.input_shape = (3,) + tuple(args.shape)
        self.flop_upper_bound = 289.61 
        self.param_upper_bound = 138.46
        self.input_shape = (3, 1000, 600)

    def get_param(self, cand):
        arch = cand

        cfg = Config.fromfile('configs/FLIR/faster_rcnn_detnas_fpn_1x_FLIR_search_subnet.py')
        cfg.model.backbone.arch = arch['backbone']
        cfg.model.roi_head.bbox_head_fusion.arch = list(arch['head_1'])
        cfg.model.roi_head.bbox_head_fusion_thermal.arch = list(arch['head_2'])
    ...
    def is_legal(self, cand):
        ...
        if fp > self.flop_upper_bound or param > self.param_upper_bound:
            return False
        # size, fp = 0, 0
        info['fp'] = fp
        info['size'] = param
    ...
    def search(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            for i, cand in enumerate(self.keep_top_k[50]):
                ...
                # else:
                logging.info('No.{} mAP = {}, cand={} '.format(
                    i + 1, self.vis_dict[cand]['mAP'], cand))
            seed(0)
