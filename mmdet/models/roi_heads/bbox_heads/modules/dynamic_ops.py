...
class DynamicInstanceNorm2d(nn.Module):
    '''
        1. doesn't acculate bn statistics, (momentum=0.)
        2. calculate BN statistics of all subnets after training
        3. bn weights are shared
    ...
    def __init__(self, max_feature_dim):
        super(DynamicInstanceNorm2d, self).__init__()
        
        self.max_feature_dim = max_feature_dim
        self.bn = nn.InstanceNorm2d(self.max_feature_dim)
        ...
        self.bn_tracking = nn.ModuleList(
            [
                nn.InstanceNorm2d(self.max_feature_dim, affine=False),
                nn.InstanceNorm2d(self.max_feature_dim, affine=False) 
            ]
        )
        ...
        if not self.need_sync:
            if bn.running_mean is None:
                return F.instance_norm(
                    x, None, None, None,
                    None, bn.training or not bn.track_running_stats,
                    bn.momentum, bn.eps,
                )
            else:
                return F.instance_norm(
                x, bn.running_mean[:feature_dim], bn.running_var[:feature_dim], None,
                None, bn.training or not bn.track_running_stats,
                bn.momentum, bn.eps,
            )
