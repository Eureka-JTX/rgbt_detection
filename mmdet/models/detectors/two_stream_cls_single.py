...
class TwoStreamClsSingle(BaseDetector):
    ...
    def __init__(self,
                ...
                #  head=None,
                 ...
                 train_cfg=None,
                 test_cfg=None):
        super(TwoStreamClsSingle, self).__init__(init_cfg)
        if pretrained:
    ...
    def extract_feat_single(self, img, img_thermal):
        """Directly extract features from the backbone+neck."""
        x = self.backbone_rgb(img)
        # x_thermal = self.backbone_thermal(img_thermal)
        # if self.with_neck:
    ...
    def simple_test(self, img, img_metas, img_thermal, proposals=None, rescale=False):
        ...
        # x, x_thermal = self.extract_feat(img, img_thermal)
        x, x_thermal = self.extract_feat_single(img, img_thermal)

        return self.head.simple_test(x, x_thermal)
