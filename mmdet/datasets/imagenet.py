...
class ClsDataset(BaseDataset):
    ...
    def load_annotations(self):
        ...
        for filename, gt_label in samples:
            ...
            # filename_thermal = filename.replace('rgb', 'thermal')
            filename_thermal = filename.replace('jpg', 'jpeg')
            info['img_info'] = {'filename': filename, 'filename_thermal': filename_thermal}
            # info['img_info'] = {'filename': filename}
            # info['gt_labels'] = np.array(gt_label, dtype=np.int64)
            info['gt_labels'] = np.array(gt_label, dtype=np.int64).reshape(-1)
