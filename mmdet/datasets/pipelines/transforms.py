    ...
    def get_params(img, scale, ratio, max_attempts=10):
        ...
        for _ in range(max_attempts):
            ...
            if 0 < target_width < width and 0 < target_height < height:
                # print(height - target_height, width - target_width)
...
class Normalize:
    ...
    def __init__(self, mean, std, to_rgb=True, thermal_mean=None, thermal_std=None):
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)

        if thermal_mean is not None:
            self.thermal_mean = np.array(thermal_mean, dtype=np.float32)
            self.thermal_std = np.array(thermal_std, dtype=np.float32)
        else:
    ...
    def __call__(self, results):
        ...
        if self.thermal_mean is not None:
            results['img_thermal'] = mmcv.imnormalize(
                                            results['img_thermal'], 
                                            self.thermal_mean, 
                                            self.thermal_std,
                                            self.to_rgb
                                            )
            results['img_norm_cfg_thermal'] = dict(
                mean=self.thermal_mean, std=self.thermal_std, to_rgb=self.to_rgb)
