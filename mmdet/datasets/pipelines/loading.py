    ...
    def __call__(self, results):
        ...
        if results['img_prefix'] is not None:
            ...
            if self.load_thermal:
                if 'rgb' in results['img_prefix']:
                    filename_thermal = osp.join(results['img_prefix'].replace('rgb','thermal'),
                                    results['img_info']['filename_thermal'])
                else:
                    filename_thermal = osp.join(results['img_prefix'],
                                    results['img_info']['filename_thermal'])
        else:
            filename = results['img_info']['filename']
            if self.load_thermal:
                filename_thermal = results['img_info']['filename_thermal']
        ...
        if self.load_thermal:
            img_bytes_thermal = self.file_client.get(filename_thermal)
            img_thermal = mmcv.imfrombytes(
                img_bytes_thermal, flag=self.color_type, channel_order=self.channel_order)
            if self.to_float32:
                img_thermal = img_thermal.astype(np.float32)
        ...
        if self.load_thermal:
            ...
            results['filename_thermal'] = filename
            results['ori_filename_thermal'] = results['img_info']['filename_thermal']
            results['img_thermal'] = img_thermal
            results['img_fields'].append('img_thermal')
